...
def execute():
    ...
    with open(performance_file, "w") as f:
        for workload in WORKLOADS:
            ...
            new_xclbin_name = f"{Mmul_4aie.__name__}_{workload[workloads.FILE_NAME_KEY].split('.mlir')[0]}.xclbin"
            new_seq_name = f"{Mmul_4aie.__name__}_{workload[workloads.FILE_NAME_KEY].split('.mlir')[0]}.seq"
                ...
                for row_a in range(0, a_shape_workload[0], a_shape_npu[0]):
                    for col_b in range(0, b_shape_workload[1], b_shape_npu[1]):
                        for col_a in range(0, a_shape_workload[1], a_shape_npu[1]):
                            ...
                            # if test > 0 and np.mean(output_c) != 32:
