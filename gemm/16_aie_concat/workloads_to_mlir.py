...
def replace_sequence_sizes(line, shapes, offset, shapes_idx):
    if shapes_idx == 0:
        ...
        pattern = r'\[%c0, %c0, (%c\d+), %c0\]\[(%c\d+), (%c\d+), (%c\d+), (%c\d+)\]\[%c0, %c0, %c0\]'
        new_shape = rf'[%c0, %c0, %c{shapes[0]//4*offset}, %c0][%c1, %c1, %c{shapes[0]//4}, %c{shapes[1]}][%c0, %c0, %c0]'
...
def process_mlir_file(input_file, output_file, inp_dtype, out_dtype, npu_shapes, kernel_shapes):
    ...
    with open(output_file, 'w') as file:
        ...
        while line is not None:
            if 'memref' in line and 'itbuffer' in line:
                ...
                line = replace_sequence_sizes(line, npu_shapes[shape_idx], itbuf_idx, shape_idx)
    ...
    required_ssa_consts_offsets_0 = {f'{shape[0]//4*i}' for shape in npu_shapes for i in range(4)}
    required_ssa_consts_offsets_1 = {f'{shape[1]//4*i}' for shape in npu_shapes for i in range(4)}
    missing_ssa_consts_offsets_0 = required_ssa_consts_offsets_0 - set(ssa_consts)
    missing_ssa_consts_offsets_1 = required_ssa_consts_offsets_1 - set(ssa_consts) - missing_ssa_consts_offsets_0
    missing_ssa_consts_kernel = required_ssa_consts_kernel - set(ssa_consts) - missing_ssa_consts_offsets_0 - missing_ssa_consts_offsets_1
    missing_ssa_consts_npu = required_ssa_consts_npu - set(ssa_consts) - missing_ssa_consts_kernel - missing_ssa_consts_offsets_0 - missing_ssa_consts_offsets_1
    if missing_ssa_consts_kernel or missing_ssa_consts_npu or missing_ssa_consts_offsets_0 or missing_ssa_consts_offsets_1:
            ...
            for line in lines:
                ...
                if sequence_found and "AIEX.ipu.dma_memcpy_nd" in line:
                    ...
                    for const in missing_ssa_consts_npu:
                        file.write(f'    %c{const} = arith.constant {const} : i32\n')
                    for const in missing_ssa_consts_offsets_0:
                        file.write(f'    %c{const} = arith.constant {const} : i32\n')
                    for const in missing_ssa_consts_offsets_1:
                        file.write(f'    %c{const} = arith.constant {const} : i32\n')
                    sequence_found = False
                file.write(line)
